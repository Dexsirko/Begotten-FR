...
-- COMMAND.tip = "Éteignez un feu sur votre corps.";
...
-- function COMMAND:OnRun(player, arguments)
	...
	-- if flames > 0 then
		...
		-- if flames == 0 then
			...
			-- Clockwork.chatBox:AddInTargetRadius(player, "me", "parvient à éteindre les flammes.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);

		-- else

			-- Clockwork.chatBox:AddInTargetRadius(player, "me", "tapote frénétiquement les flammes qui enveloppent son corps !", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);

		-- end

	-- else

		-- Clockwork.chatBox:AddInTargetRadius(player, "me", "se tape le corps comme s'il était en feu, à la manière d'un parfait imbécile.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
...
for k, v in pairs(cwAlchemy.chemicals) do
	...
	ITEM.description = "L'étiquette indique :"..v.name;
...
function cwAlchemy:DrinkChem(player,buffer)
	...
	Clockwork.player:Notify(player, "Ça a un goût"..taste..".");
	...
	if flames > 0 then
		...
		if flames > 0 then
			...
			if firewas == 0 then
				...
				Clockwork.chatBox:AddInTargetRadius(player, "me", "crie alors que les flammes enveloppent son corps !", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
			...
			if firewas > 0 then
				...
				Clockwork.chatBox:AddInTargetRadius(player, "me", "soupire de soulagement alors que le feu s'éteint.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
		...
		if transleft > 0 then
			...
			if amounttopour > 0 then
				...
				if danger.unstable > 0 then
					...
					Clockwork.chatBox:AddInTargetRadius(player, "me", "commence très prudemment à transférer le contenu de leur"..string.lower(name).." to a "..string.lower(targetname).."...", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);

				else

					Clockwork.chatBox:AddInTargetRadius(player, "me", "commence à transférer le contenu de leur"..string.lower(name).." to a "..string.lower(targetname).."...", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);

				end

				Clockwork.player:Notify(player, "Transfert en cours"..cwAlchemy:ConvertUnits(amounttopour).."...");
				...
				Clockwork.player:SetAction(player, "transferring_contents_container", transfer.total/200, 5, function()
					...
					if cwAlchemy:DisturbContainer(player, containerto, buffer.total/1000) then
						...
						Clockwork.player:Notify(player, "Votre conteneur a explosé !");
						...
						Clockwork.player:Notify(player, "Transféré"..transferred..".");

						Clockwork.chatBox:AddInTargetRadius(player, "me", "a fini de transférer le contenu vers leur"..string.lower(targetname)..".", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
				...
				Clockwork.player:Notify(player, "Le conteneur est vide.");

			end

		else

			Clockwork.player:Notify(player, "Ce conteneur est plein.");
...
function cwAlchemy:Dissolve(player,uniqueid1,itemid1,uniqueid2,itemid2)
	...
	if (dissolve and containerto) then
		...
		if transleft > 0 then
			...
			Clockwork.player:Notify(player, "Dissolution en cours"..cwAlchemy:ConvertUnits(amounttopour).."...");

			local danger = cwAlchemy:CalcExplosiveStats(buffer.output, 0)

			if danger.unstable > 0 then

				Clockwork.chatBox:AddInTargetRadius(player, "me", "place très prudemment un"..string.lower(name).." in their "..string.lower(targetname).." and watches it dissolve...", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);

			else

				Clockwork.chatBox:AddInTargetRadius(player, "me", "pose un"..string.lower(name).." in their "..string.lower(targetname).." and watches it dissolve...", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
			...
			Clockwork.player:SetAction(player, "transferring_contents_container", transfer.total/200, 5, function()
				...
				Clockwork.player:Notify(player, "Dissous un"..transferred..".");

				Clockwork.chatBox:AddInTargetRadius(player, "me", "a fini de dissoudre un"..string.lower(name)..".", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
