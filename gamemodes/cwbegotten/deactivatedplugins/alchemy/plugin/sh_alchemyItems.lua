local ITEM = Clockwork.item:New(nil, true);
	...
	ITEM.description = "Base de Conteneur";
	...
	function ITEM:OnCustomFunction(player, name)
		...
		if (name == "Chug") then
			...
			if data.volume > 0 then
				...
				Clockwork.chatBox:AddInTargetRadius(player, "me", "lève leur"..string.lower(self.name).." to their lips and starts chugging.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
				...
				Clockwork.player:Notify(player, "Vous buvez"..cwAlchemy:ConvertUnits(drink.total)..".");

			else

				Clockwork.player:Notify(player, "Ce conteneur est entièrement vide.");
		...
		elseif (name == "Dump Contents") then
			...
			if data.volume > 0 then
				...
				Clockwork.chatBox:AddInTargetRadius(player, "me", "fait un signe de tête"..string.lower(self.name).." and pours out the contents onto the ground.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
				...
				Clockwork.player:Notify(player, "Vous videz"..cwAlchemy:ConvertUnits(drink.total)..".");

			else

				Clockwork.player:Notify(player, "Ce conteneur est déjà vide !");
		...
		elseif (name == "Pour On Self") then
			...
			if data.volume > 0 then
				...
				Clockwork.chatBox:AddInTargetRadius(player, "me", "lève leur"..string.lower(self.name).." over their head and dumps the contents all over themselves!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
	...
	function ITEM:OnUse(player, itemEntity)
		...
		if data.volume > 0 then
			...
			Clockwork.chatBox:AddInTargetRadius(player, "me", "prend une gorgée de leur"..string.lower(self.name)..".", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
...
local ITEM = Clockwork.item:New(nil, true);
	...
	function ITEM:OnUse(player, itemEntity)
		...
		Clockwork.chatBox:AddInTargetRadius(player, "me", "avale un"..string.lower(self.name)..".", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
...
ITEM.description = "Une vieille jerricane en plastique.";
...
ITEM.description = "Une minuscule fiole façonnée dans une douille d'obus antique.";
...
ITEM.description = "Un jerrican de bonne taille. Il pue l'urine.";
...
ITEM.description = "Une bouteille de délicieuse lampée. Bois-la. C'est savoureux. Fais-le.";
