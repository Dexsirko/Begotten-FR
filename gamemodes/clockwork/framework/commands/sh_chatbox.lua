...
local COMMAND = Clockwork.command:New("W");
	COMMAND.tip = "Chuchote aux personnages près de toi.";
...
local COMMAND = Clockwork.command:New("Su");
	COMMAND.tip = "Envoyer un message privé à tous les superadmins.";
...
--[[local COMMAND = Clockwork.command:New("SetVoicemail");
	COMMAND.tip = "Définissez votre message vocal personnel.";
	COMMAND.text = "[string Text]";
	...
	function COMMAND:OnRun(player, arguments)
		if (arguments[1] == "none") then
			...
			Clockwork.player:Notify(player, "Vous avez supprimé votre messagerie vocale.");
		else
			player:SetCharacterData("Voicemail", arguments[1]);
			Clockwork.player:Notify(player, "Vous avez défini votre messagerie vocale sur "..arguments[1].."'.");
		end;
	end;
COMMAND:Register();]]--

local COMMAND = Clockwork.command:New("Roll");
	COMMAND.tip = "Lance un nombre entre 0 et le nombre spécifié.";
	...
	function COMMAND:OnRun(player, arguments)
		...
		if (target) then
			if (player:IsUserGroup("operator") or player:IsAdmin() or player:IsSuperAdmin()) or (target:IsUserGroup("operator") or target:IsAdmin() or target:IsSuperAdmin()) then
				...
				Clockwork.player:Notify(player, "Vous ne pouvez envoyer de messages privés qu'aux administrateurs !");
			end
		else
...
--[[local COMMAND = Clockwork.command:New("Op");
	COMMAND.tip = "Envoyer un message privé à tous les opérateurs et supérieurs.";
	COMMAND.text = "<string Msg>";
	COMMAND.access = "o";
	COMMAND.arguments = 1;
...
local COMMAND = Clockwork.command:New("Me");
	COMMAND.tip = "Parlez à la troisième personne aux personnes autour de vous.";
	COMMAND.text = "<string Text>";
	COMMAND.flags = bit.bor(CMD_DEFAULT, CMD_DEATHCODE);
	COMMAND.arguments = 1;
...
local COMMAND = Clockwork.command:New("It");
	COMMAND.tip = "Décris une action ou un événement local.";
	COMMAND.text = "<string Text>";
	COMMAND.flags = CMD_DEFAULT;
	COMMAND.arguments = 1;
	COMMAND.important = true;
...
local COMMAND = Clockwork.command:New("AdminChat");
	COMMAND.tip = "Communiquez avec les autres administrateurs.";
	COMMAND.text = "<string Text>";
	COMMAND.access = "a";
	COMMAND.arguments = 1;
	COMMAND.alias = {"Ad", "Op"};
...
COMMAND.tip = "Envoyer un message à un joueur en tant que réponse d'aide administrateur.";
...
function COMMAND:OnRun(player, arguments)
	...
	if (target) then
		...
		Clockwork.player:Notify(target, "[RÉPONSE D'AIDE ADMIN]"..message);
		Clockwork.player:Notify(Schema:GetAdmins(), "[RÉPONSE À L'AIDE ADMIN]"..player:Name().." to "..target:Name()..": "..message);
	else
