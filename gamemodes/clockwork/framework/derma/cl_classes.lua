...
function PANEL:Rebuild()
	...
	if (#classes > 0) then
		local label = vgui.Create("cwInfoText", self);
			label:SetText("Les classes que vous choisissez ne restent pas associées à votre personnage.");
		...
		local label = vgui.Create("cwInfoText", self);
			label:SetText("Vous n'avez accès à aucune classe !");
...
function PANEL:Init()
	...
	if (self.overrideData.information) then
		...
		self.information:SetText("Il y a"..players.."/"..limit.." characters with this class.");
	end;
...
function PANEL:Think()
	if (self.classTable and !self.overrideData.information) then
		self.information:SetText("Il y a".._team.NumPlayers(self.classTable.index).."/"..Clockwork.class:GetLimit(self.classTable.name).." characters with this class.");
		self.information:SizeToContents();
	end;
