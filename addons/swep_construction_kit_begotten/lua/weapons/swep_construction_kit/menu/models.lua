...
local mlabel = vgui.Create( "DLabel", pmodels )
	mlabel:SetTall( 20 )
	mlabel:SetText( "Nouvel élément de modèle de vue :" )
mlabel:Dock(TOP)
...
local pbuttons = SimplePanel( pmodels )
	...
	local rmbtn = vgui.Create( "DButton", pbuttons )
		...
		rmbtn:SetText( "Supprimer la sélection" )
	rmbtn:Dock(LEFT)
	
	local copybtn = vgui.Create( "DButton", pbuttons )
		copybtn:SetSize( 160, 25 )
		copybtn:SetText( "Copier la sélection" )
	copybtn:Dock(RIGHT)
...
local pctbtn = vgui.Create( "DButton", pmodels)
	pctbtn:SetTall( 30 )
	pctbtn:SetText("Copier la table du modèle de vue dans le presse-papiers")
...
local prtbtn = vgui.Create( "DButton", pmodels)
	prtbtn:SetTall( 30 )
	prtbtn:SetText("Afficher la table du modèle de vue dans la console")
...
local function CreateSLightningModifier( data, panel )
	...
	local lschbox = vgui.Create( "DCheckBoxLabel", panel )
		lschbox:SetText("Supprimer les éclairs du moteur")
...
local function CreateBodygroupSkinModifier( data, panel )
	...
	local bdlabel = vgui.Create( "DLabel", panel )
		bdlabel:SetText( "Groupe corporel :" )
...
local mlabel = vgui.Create( "DLabel", pwmodels )
	mlabel:SetTall( 20 )
	mlabel:SetText( "Nouvel élément de modèle de monde :" )
mlabel:Dock(TOP)
...
local pwbuttons = SimplePanel( pwmodels )
	...
	local importbtn = vgui.Create( "DButton", pwbuttons )
		...
		importbtn:SetText( "Importer les modèles de vue" )
	importbtn:Dock(FILL)
...
local pctbtn = vgui.Create( "DButton", pwmodels)
	pctbtn:SetTall( 30 )
	pctbtn:SetText("Copier la table des modèles du monde dans le presse-papiers")
	pctbtn.DoClick = function()
...
local prtbtn = vgui.Create( "DButton", pwmodels)
	prtbtn:SetTall( 30 )
	prtbtn:SetText("Afficher la table des modèles du monde dans la console")
	prtbtn.DoClick = function()
		MsgN("*********************************************")
		for k, v in pairs(string.Explode("\n",GetWModelsText())) do
			MsgN(v)
